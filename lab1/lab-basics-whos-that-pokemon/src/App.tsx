...
// Data (1..151 meegegeven, maar we gebruiken de eerste 150 zoals in de opdracht)
...
function pickRandomUnique<T>(arr: T[], count: number): T[] {
  const idxs = new Set<number>();           // <— belangrijk: Set<number>
  const n = Math.min(count, arr.length);
  while (idxs.size < n) {
    idxs.add(Math.floor(Math.random() * arr.length));
  }
  // Array.from met mapping zodat i als number wordt getypt
  return Array.from(idxs, (i) => arr[i]);
}
...
function App() {
  // gebruik alleen de eerste 150
  const list = useMemo<Pokemon[]>(() => pokemons.slice(0, 150), []);

  // willekeurige target
  const target = useMemo<Pokemon>(() => {
    const t = list[Math.floor(Math.random() * list.length)];
    // non-null assertion omdat list lengte > 0 is
    return t!;
  }, [list]);

  // 3 afleiders + juiste antwoord, dan schudden
  const options = useMemo<Pokemon[]>(() => {
    const others = list.filter((p) => p.id !== target.id);
    const distractors = pickRandomUnique(others, 3);
    return shuffle([target, ...distractors]);
  }, [list, target]);

  const imageUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${target.id}.png`;

  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6">
      <div className="max-w-xl mx-auto space-y-6">
        <header>
          <h1 className="text-2xl font-bold">lab-basics-whos-that-pokemon</h1>
          <p className="text-sm text-gray-600">Toon een willekeurige Pokémon + 4 willekeurige namen (knoppen doen nog niets).</p>
        </header>

        <figure className="rounded-2xl border bg-white p-6 flex flex-col items-center gap-4">
          <img
            src={imageUrl}
            alt="Who's that Pokémon?"
            style={{ filter: "brightness(0)", width: 256, height: 256, objectFit: "contain" }}
          />
          <figcaption className="text-sm text-gray-700">Wie is deze Pokémon?</figcaption>
        </figure>

        <div className="grid grid-cols-2 gap-3">
          {options.map((p) => (
            <button key={p.id} type="button" className="rounded border px-3 py-2 bg-white">
              {p.name}
            </button>
          ))}
        </div>

        <section className="bg-white rounded-2xl border p-4 text-sm text-gray-700">
          <h2 className="font-semibold mb-1">Opmerking</h2>
          <ul className="list-disc list-inside">
            <li>De afbeelding wordt geladen via de officiële artwork-URL met het <code>id</code>.</li>
            <li>De identiteit is verborgen met <code>filter: brightness(0)</code>.</li>
            <li>Knoppen zijn willekeurige namen; functionaliteit volgt later.</li>
          </ul>
        </section>
      </div>
    </div>
